...
const runWorkflow = async (pullRequestLink: string): Promise<boolean> => {
  ...
  try {
    ...
    if (!workflow) throw new Error("automationTestWorkflow not found");
...
(async () => {
  ...
  const isCompletedSuccessfully = await runWorkflow(pullRequestLink);

  await cleanup();

  process.exit(isCompletedSuccessfully ? 0 : 1);
})();
